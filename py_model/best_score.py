...
# Feature Engineering
group_ticket = train.groupby(by = "Ticket", as_index=False).agg({"PassengerId" : 'count', "Sex" : lambda x : x[x == "female"].count()})
group_ticket.columns = ["Ticket", "SameTicket", "FemalesOnTicket"]
data = pd.merge(data, group_ticket, how = "left", on = "Ticket")
...
group_title = data.groupby(by = "Title", as_index = False).agg({"Survived" : 'mean'})
group_title.columns = ["Title", "TargetByTitle"]
data = pd.merge(data, group_title, how = "left", on = "Title")

data["Family"] = data["Parch"] + data["SibSp"] + 1
data["Alone"] = data["Family"] == 1

group_cabin = train.groupby(by = "Cabin", as_index = False).agg({"PassengerId" : 'count'})
group_cabin.columns = ["Cabin", "SameCabin"]
data = pd.merge(data, group_cabin, how = "left", on = "Cabin")
...
group_deck = data.groupby(by = "Deck", as_index = False).agg({"Survived" : 'mean'})
group_deck.columns = ["Deck", "TargetByDeck"]
data = pd.merge(data, group_deck, how = "left", on="Deck")
...
group_pclass = data.groupby(by = "Pclass", as_index = False).agg({"Survived" : 'mean'})
group_pclass.columns = ["Pclass", "TargetByPclass"]
data = pd.merge(data, group_pclass, how = "left", on = "Pclass")

group_embarked = data.groupby(by = "Embarked", as_index = False).agg({"Survived" : 'mean'})
group_embarked.columns = ["Embarked", "TargetByEmbarked"]
data = pd.merge(data, group_embarked, how = "left", on = "Embarked")

group_sex = data.groupby(by = "Sex", as_index = False).agg({"Survived" : 'mean'})
group_sex.columns = ["Sex", "TargetBySex"]
data = pd.merge(data, group_sex, how = "left", on = "Sex")
